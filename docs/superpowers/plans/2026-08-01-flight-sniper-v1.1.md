...
**Goal:** Превратить пассивный дайджест в активного сторожа: подтверждать аномалии realtime-ценой Amadeus, слать мгновенные BUY-алерты, ловить кросс-рыночный арбитраж и принимать команды из Telegram.

**Architecture:** Слой Amadeus подключается только по требованию — на кандидатов-аномалий и как fallback при пустом кэше, с жёстким счётчиком квоты в SQLite. Детекция аномалий работает по локальной валюте внутри рынка (чтобы движение курса не выглядело как падение цены), вердикт и арбитраж — по `landed_usd`. Команды обрабатываются лёгким воркфлоу каждые 15 минут через `getUpdates`; изменяемое состояние живёт в таблице `meta`.

**Tech Stack:** тот же, что в v1. Новое: Amadeus Self-Service API (OAuth2 client_credentials + Flight Offers Search v2), Telegram getUpdates.
...
|---|---|
| `src/sources/amadeus.py` | **новый** — OAuth, Flight Offers Search, парсинг офферов |
| `src/quota.py` | **новый** — учёт вызовов внешних API, hard stop |
| `src/alerts.py` | **новый** — кандидаты → подтверждение → дедуп → текст алерта |
