...
def insert_prices(conn: sqlite3.Connection, records: Iterable[PriceRecord], now: str) -> int:
    """Вставляет записи. Идентичный оффер не дублируется.

    У повторно увиденного оффера обновляются last_seen_at и пересчитанные по
    свежему курсу fx_rate/landed_usd — иначе курс замёрз бы на момент первой
    встречи и перестал отличать движение цены от движения валюты.
